...
############################################################################################################################
    ...
    opsd_load *= scaling
    logger.info(f"Load data scaled with time series")
    if snakemake.config.get('load').get('time_adjustments'):
    	print('snakemake.config.get hat geklappt')
    	opsd_load = variing_adjustments(opsd_load)
    logger.info(f'time_adjustments finished')
...
####################################
    ...
    Nyears = n.snapshot_weightings.values.sum()/8760 #n.snapshot_weightings.objective.sum() / 8760.############################# changed, as 'objective' wasn't found anywere
