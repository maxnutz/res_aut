...
install = False config.get('install_packages')
if install == True:
...
jahr = config.get('network').get('year')
...
def Residuallast_per_day (jahr, silvester, pres, einjahr): 
    ...
    plot_Residuallast(counts, 'Tag', 'd')
...
def Nulldurchgang_per_day(jahr, res, silvester, einjahr):
    ...
    plot_Nulldurchgang(counts, months, einjahr)
...
def Lastverschiebung_einfach(res):
    #Residuallast der Lastverschiebung
    v = 0
    w = 0
    lastv = 0
    RES = 0
    EE = 0 
    cols = ['Anfang', 'Ende', 'Delta E [MWh]']
    allframe = pd.DataFrame(columns = cols)
    series = pd.Series(dtype = float)
    for v in range(1, res.size):
        if (res['AT0 0'].iloc[v-1] > 0 and res['AT0 0'].iloc[v] <= 0):
            RES = count_power(w, v)
            series = series.append(pd.Series([RES], index = [w])) #RES
            for w in range(v, res.size):
                if res['AT0 0'].iloc[w-1] <= 0 and res['AT0 0'].iloc[w] > 0:
                    EE = count_power(v, w)
                    series = series.append(pd.Series([EE], index = [v])) #EE
                    df2 = pd.DataFrame([[lastv, v, (EE + RES)]], columns = cols)
                    allframe = allframe.append(df2, ignore_index = True)
                    lastv = v
                    break
    allframe['Delta t'] = allframe['Ende'] - allframe['Anfang']
    allframe['Delta E [MWh]'].iloc[0] = 0
    allframe['P/t [MW]'] = allframe['Delta E [MWh]'] / allframe['Delta t']
    plot_Lastverschiebung(allframe, allframe['P/t [MW]'], 'Residuallast bei Lastverschiebung innerhalb eines Zyklus', 'green')
    #Last, die verschoben werden kann
    LVp = []
    for v in range(0, allframe.index.size):
        a = allframe['Anfang'].loc[v]
        for b in range(a, res.size):
            if res['AT0 0'].iloc[b] < 0 and res['AT0 0'].iloc[b+1] >= 0:
                EEver = abs(int(res.iloc[a:b].sum()))
                break
        if a < 0:
            EEver -= allframe['Anfang'].loc[v]
        LVp.append(EEver)
    allframe['EE verschoben [MWh]'] = LVp
    allframe['EE verschoben/t [MW]'] = allframe['EE verschoben [MWh]'] / allframe['Delta t']
    plot_Lastverschiebung(allframe, allframe['EE verschoben/t [MW]'], 'Energie, die innerhalb eines Zyklus verschoben wird', 'orange')
    allframe.to_csv(ordner + 'Lastverschiebung_einfach.csv')
    with pd.ExcelWriter(excel_path, mode = 'a') as writer:
        allframe.to_excel(writer, 
                       sheet_name = 'LV - Residuallast')
        
def plot_Lastverschiebung(allframe, data, title, farbe):
    fig, ax = plt.subplots(figsize=(20,5))
    ax.plot([0, 10],[0, 10], linewidth = 0.03) #doesn't work without
    ax.set_yscale('symlog')
    for v in range(0, allframe.index.size):
        if farbe != 'orange':
            farbe = 'green' if allframe['P/t [MW]'].loc[v] < 0 else 'red'
        ax.add_patch(Rectangle((allframe['Anfang'].loc[v], 0),
                               allframe['Ende'].loc[v] - allframe['Anfang'].loc[v],
                               data.loc[v], color = farbe))
    plt.title(title, fontsize = 23)
    xt = []
    for i in range(0, 12):
        xt.append(int(i/12 * allframe['Ende'].loc[allframe.index.size-1]))
    ax.set_xticks(xt)
    ax.set_xticklabels(months, minor = False, rotation = 45)
    plt.xlabel('Zeit [h]')
    plt.ylabel('Leistung [MW]')
    plt.savefig(ordner + '/Plots/Lastverschiebung/' + title +'.png',
                   format = 'png',
                   bbox_inches = 'tight')
    plt.close()
    # Plot für Lastverschiebung (runs automatically!)
